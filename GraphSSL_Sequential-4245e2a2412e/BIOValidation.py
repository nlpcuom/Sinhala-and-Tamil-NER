...
def convert(predictions):
    ...
    for sentenceIdx in range(len(predictions)):
        ...
        for labelIdx in predictions[sentenceIdx]:
            if(labelIdx == 'O'):
                convert.append('O')
                labelStarted = False
            elif(labelStarted and labelClass == labelIdx):
                convert.append('I-' + labelIdx)
            elif(labelStarted and labelClass != labelIdx):
                convert.append('B-' + labelIdx)
                labelClass = labelIdx
            else:
                labelClass = labelIdx
                labelStarted = True
...
def compute_f1(predictions, correct, idx2Label,  correctBIOErrors='No', encodingScheme='BIO'):
    ...
    label_pred = convert(label_pred)
    label_correct = convert(label_correct)
...
def testEncodings():
    ...
    goldBIO = [['O', 'B-PER', 'I-PER', 'O', 'B-PER', 'B-PER', 'I-PER'], ['O', 'B-PER', 'B-LOC', 'I-LOC', 'O', 'B-PER',
                                                                         'I-PER', 'I-PER'], ['B-LOC', 'I-LOC', 'I-LOC', 'B-PER', 'B-PER', 'I-PER', 'I-PER', 'O', 'B-LOC', 'B-PER']]

    print("--Test IOBES--")
    goldIOBES = [['O', 'B-PER', 'E-PER', 'O', 'S-PER', 'B-PER', 'E-PER'], ['O', 'S-PER', 'B-LOC', 'E-LOC', 'O', 'B-PER',
                                                                           'I-PER', 'E-PER'], ['B-LOC', 'I-LOC', 'E-LOC', 'S-PER', 'B-PER', 'I-PER', 'E-PER', 'O', 'S-LOC', 'S-PER']]
    ...
    for sentenceIdx in range(len(goldBIO)):
        for tokenIdx in range(len(goldBIO[sentenceIdx])):
            assert(goldBIO[sentenceIdx][tokenIdx] ==
                   goldIOBES[sentenceIdx][tokenIdx])

    print("--Test IOB--")
    goldIOB = [['O', 'I-PER', 'I-PER', 'O', 'I-PER', 'B-PER', 'I-PER'], ['O', 'I-PER', 'I-LOC', 'I-LOC', 'O', 'I-PER',
                                                                         'I-PER', 'I-PER'], ['I-LOC', 'I-LOC', 'I-LOC', 'I-PER', 'B-PER', 'I-PER', 'I-PER', 'O', 'I-LOC', 'I-PER']]
...
def compute_argument_chunk_precision(guessed_sentences, correct_sentences):
    ...
    for sentenceIdx in range(len(guessed_sentences)):
        assert(len(guessed_sentences[sentenceIdx])
               == len(correct_sentences[sentenceIdx]))
