...
class ITML(BaseMetricLearner):
    ...
    def _process_inputs(self, X, constraints, bounds):
        ...
        # init bounds
        if bounds is None:
            print('init pairwise_distances')
            #distance=euclidean_distances(X,X, inverse=False)
            dist = distance.cdist(X[:30000],X[:30000], 'euclidean' )
            print('init percentile')
            self.bounds_ = np.percentile(dist, q=(5, 95))
            print(self.bounds_)
        else:
            assert len(bounds) == 2
            self.bounds_ = bounds
    ...
    def fit(self, X, constraints, bounds=None):
        ...
        for it in xrange(self.max_iter):
            ...
            for i, v in enumerate(pos_vv):
                ...
                alpha = min(_lambda[i], gamma_proj *
                            (1. / wtw - 1. / pos_bhat[i]))
                _lambda[i] -= alpha
