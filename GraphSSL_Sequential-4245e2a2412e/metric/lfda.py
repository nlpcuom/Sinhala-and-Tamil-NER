...
class LFDA(BaseMetricLearner):
    ...
    def _process_inputs(self, X, y):
        ...
        if self.k is None:
            ...
            warnings.warn(
                'Chosen k (%d) too large, using %d instead.' % (self.k, d - 1))
            k = d - 1
        else:
            k = int(self.k)
    ...
    def fit(self, X, y):
        ...
        for c in xrange(num_classes):
            ...
            #dist = pairwise_distances(Xc, metric='l2', squared=True, n_jobs=-1)
            dist = distance.cdist(Xc, Xc, 'euclidean')
            print('Calculating Pairwise, iter: ', c)
